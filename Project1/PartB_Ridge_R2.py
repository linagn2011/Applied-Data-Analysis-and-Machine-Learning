...
Ridge_r2_train = np.zeros(N)
Ridge_r2_pred = np.zeros(N)
...
for i, deg in enumerate(degrees):
    ...
    for j, lmbda in enumerate(lambdas):
        ...
        Ridge_r2_pred[j] = r2(z_pred,z_test_scaled)
        Ridge_r2_train[j] = r2(z_tilde, z_train_scaled)

    ax = fig.add_subplot(2, 2, i+1)
    ax.grid(alpha=0.3)
    ax.plot(np.log10(lambdas),Ridge_r2_pred, label='R^2_test')
    ax.plot(np.log10(lambdas),Ridge_r2_train, label='R^2_train')
    ax.set_xlabel(r"$\log_{10} \lambda$")
    ax.set_ylabel("Error")
    ax.set_title(f"Model Complexity {deg}")
    ax.legend()
    
print(Ridge_r2_pred,Ridge_r2_train)
    
plt.tight_layout()
plt.show()
fig.savefig(fig_path("Ridge_lamdba_R2.jpg"), dpi=300, transparent=True)
