...
for i, deg in enumerate(degrees):
    ...
    for j, lmbda in enumerate(lambdas):
        ...
        Lasso_mse_pred[j] = mse(z_test_scaled, z_pred)
        Lasso_mse_train[j] = mse(z_test_scaled, z_tilde)

    ax = fig.add_subplot(2, 2, i+1)
    ax.grid(alpha=0.3)
    ax.plot(np.log10(lambdas),Lasso_mse_pred, label='MSE_test')
    ax.plot(np.log10(lambdas),Lasso_mse_train, label='MSE_train')
...
plt.tight_layout()
plt.show()
fig.savefig(fig_path("lasso_lamdba.jpg"), dpi=300, transparent=True)
